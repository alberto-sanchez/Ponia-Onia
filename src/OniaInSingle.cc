// -*- C++ -*-
//
// Package:    OniaInSingle
// Class:      OniaInSingle
// 
// Description: Dump  Onia(mu+ mu-)  decays for rho factor studies
//
...
class OniaInSingle:public edm::EDAnalyzer {
	...
	static void fillDescriptions(edm::ConfigurationDescriptions & descriptions);
      ...
      private:
        UInt_t getDimuonTriggerBits(const edm::Event &);
...
OniaInSingle::OniaInSingle(const edm::ParameterSet & iConfig):
...
prop2_(iConfig.getParameter<edm::ParameterSet>("propagatorStation2"))
{
  ...
  if (isMC_ || OnlyGen_) {
     std::cout << "OniaInSingle::OniaInSingle: Onia id " << pdgid_ << std::endl;
...
OniaInSingle::~OniaInSingle() {}

//
// member functions
//

const reco::Candidate* OniaInSingle::GetAncestor(const reco::Candidate* p) {
...
bool OniaInSingle::isAncestor(const reco::Candidate* ancestor, const reco::Candidate * particle) {
...
UInt_t OniaInSingle::getDimuonTriggerBits(const edm::Event& iEvent ) {
   UInt_t itrigger = 0;
   edm::Handle<edm::TriggerResults> triggerResults_handle;
   iEvent.getByToken(triggerResults_Label, triggerResults_handle);
   if ( triggerResults_handle.isValid() ) {
...
void OniaInSingle::analyze(const edm::Event & iEvent, const edm::EventSetup & iSetup) {
  ...
  trigger = getDimuonTriggerBits(iEvent);
  ...
  if ( (isMC_ || OnlyGen_) && packed.isValid() && pruned.isValid() ) {
    ...
    if ( ! dimuon_pdgId ) std::cout << "OniaInSingle: does not found the given decay " << run << "," << event << std::endl; // sanity check
  }  // end if isMC
...
UInt_t OniaInSingle::isTriggerMatched(const pat::CompositeCandidate *diMuon_cand) {
...
void OniaInSingle::muonStationDistance (const pat::CompositeCandidate* aCand) {
...
void OniaInSingle::endJob() {}

// ------------ method called when starting to processes a run  ------------
void OniaInSingle::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup) {
    prop1_.init(iSetup);
    prop2_.init(iSetup);
}

// ------------ method called when ending the processing of a run  ------------
void OniaInSingle::endRun(edm::Run const &, edm::EventSetup const &) {}

// ------------ method called when starting to processes a luminosity block  ------------
void OniaInSingle::beginLuminosityBlock(edm::LuminosityBlock const &, edm::EventSetup const &) {}

// ------------ method called when ending the processing of a luminosity block  ------------
void OniaInSingle::endLuminosityBlock(edm::LuminosityBlock const &, edm::EventSetup const &) {}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void OniaInSingle::fillDescriptions(edm::ConfigurationDescriptions & descriptions) {
...
DEFINE_FWK_MODULE(OniaInSingle);
